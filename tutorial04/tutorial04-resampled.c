 ...
 *
 *           Compiled using
 *               gcc -o tutorial04-resampled tutorial04-resampled.c -lavutil
 ...
 *           tutorial04-resampled-deprecated.c for the implementation using
 *deprecated APIs.
 *
...
void printHelpMenu() {
  ...
  printf(
      "e.g: ./tutorial04-resampled /home/rambodrahmani/Videos/video.mp4 200\n");
}
 ...
 * AVStream index. For the AVMEDIA_TYPE_AUDIO codec type sets the desired audio
 * specs, opens the audio device and starts playing.
 *
  ...
  switch (codecCtx->codec_type) {
    ...
    case AVMEDIA_TYPE_VIDEO: {
      ...
      screen = SDL_CreateWindow(
          ...
          SDL_WINDOWPOS_UNDEFINED, codecCtx->width / 2, codecCtx->height / 2,
      ...
      videoState->texture = SDL_CreateTexture(
          videoState->renderer, SDL_PIXELFORMAT_YV12,
          SDL_TEXTUREACCESS_STREAMING, videoState->video_ctx->width,
  ...
  for (;;) {
    ...
    while (audio_pkt_size > 0) {
      ...
      if (got_frame) {
        // audio resampling
        data_size =
            audio_resampling(videoState->audio_ctx, avFrame, AV_SAMPLE_FMT_S16,
                             videoState->audio_ctx->channels,
                             videoState->audio_ctx->sample_rate, audio_buf);
 ...
 * specified base, which is returned as a long int value. If endptr is not a
 * null pointer, the function also sets the value of endptr to point to the
