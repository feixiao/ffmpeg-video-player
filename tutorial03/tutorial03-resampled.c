 ...
 *
 *           Compiled using
 *               gcc -o tutorial03-resampled tutorial03-resampled.c -lavutil
 *-lavformat -lavcodec -lswscale -lswresample -lz -lm  `sdl2-config --cflags
 *--libs` on Arch Linux.
 *
 *           Please refer to previous tutorials for uncommented lines of code.
 *           This implementation uses new APIs. Please refer to
 *           tutorial03-resampled-deprecated.c for the implementation using
 *deprecated APIs.
 *
...
int main(int argc, char *argv[]) {
  ...
  ret = avcodec_parameters_to_context(
      aCodecCtx, pFormatCtx->streams[audioStream]->codecpar);
  if (ret != 0) {
  ...
  SDL_Window *screen = SDL_CreateWindow(
      "FFmpeg SDL Video Player", SDL_WINDOWPOS_UNDEFINED,
      SDL_WINDOWPOS_UNDEFINED, pCodecCtx->width / 2, pCodecCtx->height / 2,
    ...
    if (pPacket->stream_index == videoStream) {
      ...
      while (ret >= 0) {
        ...
        if (++frameIndex <= maxFramesToDecode) {
          ...
          //       pCodecCtx->frame_number, pFrame->pts, pFrame->pkt_dts,
          //       pFrame->key_frame, pFrame->coded_picture_number,
          //       pFrame->display_picture_number, pCodecCtx->width,
          //       pCodecCtx->height);
          ...
          SDL_UpdateYUVTexture(texture, &rect, pict->data[0], pict->linesize[0],
...
void printHelpMenu() {
  ...
  printf(
      "e.g: ./tutorial03-resampled /home/rambodrahmani/Videos/video.mp4 200\n");
}
 ...
 * queue is empty, 1 if it is not empty and a packet was extract (pkt)
 */
static int packet_queue_get(PacketQueue *q, AVPacket *pkt, int block) {
  int ret;
 ...
 *
 * @param   audio_decode_ctx    the audio codec context retrieved from the
 * original AVFormatContext.
 * @param   decoded_audio_frame the decoded audio frame.
 * @param   out_sample_fmt      audio output sample format (e.g.
 * AV_SAMPLE_FMT_S16).
 * @param   out_channels        audio output channels, retrieved from the
 * original audio codec context.
 * @param   out_sample_rate     audio output sample rate, retrieved from the
 * original audio codec context.
...
static int audio_resampling(  // 1
  ...
  if (ret < 0) {
    printf(
        "av_samples_alloc_array_and_samples() error: Could not allocate "
        "destination samples.\n");
    return -1;
  }
 ...
 *
 * Function Parameters:
 *    device:           a UTF-8 string reported by SDL_GetAudioDeviceName();
 ...
 * is a UTF-8 string reported by SDL_GetAudioDeviceName(), but some drivers
 * allow arbitrary and driver-specific strings, such as a hostname/IP address
 * for a remote audio server, or a filename in the diskaudio driver. iscapture:
 * non-zero to specify a device should be opened for recording, not playback
 *    desired:          an SDL_AudioSpec structure representing the desired
 * output format; see SDL_OpenAudio() for more information obtained:         an
 * SDL_AudioSpec structure filled in with the actual output format; see
 * SDL_OpenAudio() for more information allowed_changes:  0, or one or more
 * flags OR'd together; These flags specify how SDL should behave when a device
 * cannot offer a specific feature. If the application requests a feature that
 * the hardware doesn't offer, SDL will always try to get the closest
 * equivalent. For example, if you ask for float32 audio format, but the sound
 * card only supports int16, SDL will set the hardware to int16. If you had set
 *                      SDL_AUDIO_ALLOW_FORMAT_CHANGE, SDL will change the
 * format in the obtained structure. If that flag was not set, SDL will prepare
 * to convert your callback's float32 audio to int16 before feeding it to the
 * hardware and will keep the originally requested format in the obtained
 * structure. If your application can only handle one specific data format, pass
 * a zero for allowed_changes and let SDL transparently handle any differences.
 *
