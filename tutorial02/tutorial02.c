 ...
 *
 *           Compiled using
 *               gcc -o tutorial02 tutorial02.c -lavutil -lavformat -lavcodec
 *-lswscale -lz -lm  `sdl2-config --cflags --libs` on Arch Linux. sdl-config
 *just prints out the proper flags for gcc to include the SDL libraries
 *properly. You may need to do something different to get it to compile on your
 *system; please check the SDL documentation for your system. Once it compiles,
 *go ahead and run it.
 *
...
int main(int argc, char *argv[]) {
   ...
   * Plese refer to tutorial02-deprecated.c for an implementation using the
   * deprecated SDL API.
   */
    ...
    if (pPacket->stream_index == videoStream) {
      ...
      while (ret >= 0) {
        ...
        if (++i <= maxFramesToDecode) {
          ...
          // which holds the position and w, h which holds width and height.It's
          // important to note that 0, 0 is the upper-left corner in SDL. So a
          // higher y-value means lower, and the bottom-right corner will have
          // the coordinate x + w, y + h.
          SDL_Rect rect;
          rect.x = 0;
          rect.y = 0;
          ...
          SDL_UpdateYUVTexture(
              ...
              &rect,  // a pointer to the rectangle of pixels to update, or NULL
